...
class Empresa{
    ...
    public function __construct($denominacion,$direc,$colClientes,$colMotos,$ventasRealizadas){
    ...
    public function cantVentas(){
        ...
        return "Cantidad de ventas: ".$cantidad."\n";
    }
    ...
    public function __toString(){
        ...
        ."\n"."Ventas realizadas: ".$this->cantVentas()."\n";
    }
    ...
    public function registrarVenta($colCodigosMoto, $objCliente){
       //no tiene que calcular el precio eso lo hace incorporarMoto()
        ...
        if($objCliente->getDadoDeBaja()){
            ...
            if($importeFinal>0){
                $copiaColVentas=$this->getVentasRealizadas();//si se pudo registrar el precio el objeto moto se guarda en la coleccion motos
                ...
                $this->setVentasRealizadas($copiaColVentas);
            }
    ...
    */
    public function retornarVentasXCliente($tipo, $dni){
        $arrHistorialVentas = $this->getVentasRealizadas();
        $colVentasCliente = [];
        
        foreach($arrHistorialVentas as $unaVenta) {
            $clienteVenta = $unaVenta->getRefCliente();
            if($clienteVenta->getTipo() == $tipo && $clienteVenta->getDni() == $dni) {
